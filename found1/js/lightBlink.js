class lightBlink {
  constructor({ direction = 'left', color = { h:300,s:100,b:100}, w = 50, h = 100, x = 200, y = 230, distance = 100, speed = 1, loop = 0, alpha = 1, solid = 1 } = {})   {
      ...
      if(this.direction == 'right' || this.direction == 'left'){
        if(this.direction == 'right'){
          ...
          if(i <= this.height / solidSize){
            ...
            alpha = map(this.x + i, this.x + this.height - this.height / solidSize , this.x + this.height , alphaStart, alphaEnd); 
          
          }else if( i > this.height / solidSize && i < this.height -this.height / solidSize){
              alpha = map(this.x + i, this.x + this.height / solidSize , this.x + this.height - this.height / solidSize , this.alpha, this.alpha); 
          }
        ...
        fill(this.startColor.h,this.startColor.s,this.startColor.b,alpha);
        rect(this.x + i , this.y , 1, this.width);
      } else {
        if(this.direction == 'up'){
          
          if(i <= this.height / solidSize){
             alpha = map(this.y + i, this.y , this.y + this.height / solidSize , alphaEnd, alphaStart); 
          }
          else if (i >= this.height - this.height / solidSize ){
            alpha = map(this.y + i, this.y + this.height - this.height / solidSize , this.y + this.height , alphaStart, alphaEnd); 
          
          }else if( i > this.height / solidSize && i < this.height -this.height / solidSize){
              alpha = map(this.y + i, this.y + this.height / solidSize , this.y + this.height - this.height / solidSize , this.alpha, this.alpha); 
          }
