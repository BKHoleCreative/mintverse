...
async function get_wallet_info_web3() {
    ...
    // get claimed number
    // const claimedNumber = await WordContract.methods.wlmintamt(connectedAddress[0]).call({});
...
async function mint_whitelist_word(wallet_info) {
    ...
    const verification = await WordContract.methods.verify(wallet_info.signedSignature).call({from:connectedAddress[0]});
